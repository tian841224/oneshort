...
5. **z-index 走既定 scale**：固定／浮動層級用 `--z-*` token（`--z-nav / --z-content-overlay / --z-bottom-nav / --z-overlay / --z-float-window / --z-dropdown / --z-modal / --z-toast`），不要寫任意數字。其中 `--z-float-window`（攻略／小工具浮動視窗與最小化 dock）介於 `--z-overlay` 與 `--z-dropdown` 之間；同層多視窗的堆疊順序一律由 DOM 順序（portal host 內排序）決定，不寫逐視窗 inline z-index。
