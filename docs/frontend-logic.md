...
9. 未登入瀏覽尋找隊伍時，大廳聊天仍啟用 `lobby:chat` 訂閱、載入 lobby chat history API（最近 100 筆、最久 24 小時），只讀不需任何角色設定。訪客嘗試「送出」第一則訊息時，若尚未透過既有 quick-guest session 設定過角色資訊（暱稱＋職業＋等級），先彈出 `GuestProfilePrompt`（dialog variant）要求設定，未完成無法送出；設定完成後（含同一瀏覽器之後的訊息，cookie 有效期內免重填）發言者顯示等級與職業 pill，比照登入玩家，不再是通用「遊客」樣式（ADR-0032）。登出後需清掉帳號特定的聊天室草稿/角色選擇/「我」標示，但保留公開大廳聊天可用。大廳聊天送出後前端需顯示倒數並停用送出按鈕：未登入 10 秒、登入後 5 秒；若後端回 `429` 與 `Retry-After`，以前端倒數顯示剩餘秒數並保留草稿。當 lobby chat WebSocket 未連線時，聊天室指示燈顯示紅色，訊息輸入與送出按鈕必須停用，避免使用者以為已送出但實際沒有連線。
...
[訪客/未登入]:
  ...
  - 一般隊伍不顯示獨立「加入此隊伍」欄位；符合資格的空缺列不再顯示「可以加入」文字 pill，改以左側圖示（含 tooltip）+ 右側「加入」CTA + 整列可點擊表示；不符合資格顯示「條件不符」、未登入且不可快速加入顯示「登入後申請」文字狀態
