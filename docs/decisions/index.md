...
| [ADR-0029](0029-party-card-type-chip-reinstated.md) | 尋找隊伍卡片／預覽面板的「目標」旁重新加回隊伍類型 chip（抽出共用 `PartyTypeChip` 元件），推翻 frontend repo `docs/frontend-logic.md` 一條早於 ADR 制度、無正式編號的既有決策（類型改由左側 Logo 傳達） | Accepted | 2026-07-11 | frontend / party |
...
| [ADR-0031](0031-guild-settings-button-visibility.md) | 公會設定入口改為主分頁列常駐分頁（依 `isOfficer` 條件附加第 6 個分頁），移除 kebab overflow 選單與其專屬 state/CSS，使設定入口不再依賴斷點隱藏 | Accepted | 2026-07-11 | frontend / guild |
| [ADR-0032](0032-lobby-chat-guest-profile.md) | 大廳聊天訪客職業/等級顯示複用既有 quick-guest Redis profile 機制（`SendLobbyChatMessage` 接上 `EnsureQuickGuestProfile`／cookie，渲染短路條件改為僅看等級是否已設定，職業含初心者 0 皆不影響），推翻方案 B（獨立訪客機制）與方案 C（前端直傳無 session）。編號跳過 0031——該號已被另一支並行分支（公會設定入口）搶先使用，避免撞號 | Accepted | 2026-07-11 | backend / notify / party；frontend / chat / party |
