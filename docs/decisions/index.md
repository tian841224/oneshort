...
| [ADR-0018](0018-create-party-desktop-wizard.md) | 建立隊伍桌機版改為分步 Step Wizard：抽出裝置無關的 `useWizardFieldErrors`/`CreatePartyWizardNav`，`.os-create-party-layout` 從 3 欄收斂為 2 欄（wizard | 常駐預覽） | Accepted | 2026-07-09 | frontend / party |
| ADR-0019 | **本分支缺此文件，非本次任務範圍**：程式碼內多處註解引用「ADR-0019」（訪客身分跨元件同步、Navbar 訪客身分小圓標、`useCloseOnOutsideClick` 共用邏輯——見 `src/lib/quickGuestIdentity.ts`、`src/hooks/useQuickGuestProfile.ts`、`src/hooks/useCloseOnOutsideClick.ts`、`src/components/shell/Navbar.tsx`、`src/components/shell/authControls.test.tsx`）。經查證，該文件其實已存在——`docs/create-party-desktop-wizard` 分支（commit e5d3fbc）已提交 `0019-guest-identity-reactivity-and-navbar-parity.md`，只是尚未合併進本分支／develop，故此處索引暫時看不到它，待該分支合併進 develop 後即會出現。**跨分支 ADR-0018 內容重複已解決**：同一 commit e5d3fbc 也曾提交一份與本分支 [ADR-0018](0018-create-party-desktop-wizard.md) 同編號但範圍/措辭不同的 `0018-create-party-desktop-wizard.md`，兩者描述的其實是同一個決策；已將對方版本的獨有細節合併進本分支的 ADR-0018，並移除該分支上的重複檔案，本分支的 [ADR-0018](0018-create-party-desktop-wizard.md) 為唯一權威版本。 | 待跨分支合併（僅缺本文件） | — | frontend / auth |
...
| ADR-0022 | **本分支缺此文件，非本次任務範圍**：訪客建立隊伍畫面新增手機版 step wizard 送出入口，修正手機版無法觸及送出按鈕的問題。文件位於尚未合併進本分支的 `docs/guest-mobile-create-submit` 分支（`0022-guest-create-party-mobile-submit-entry.md`）——原編號 ADR-0017，因與本分支已合併進 backend develop 的 [ADR-0017](0017-job-classes-seed-cleanup.md) 撞號而改編號為 0022；此列僅為預先登記，避免未來再有分支獨立搶用 0022。 | Accepted（另一分支） | 2026-07-09 | frontend-rwd / party / auth |
