...
| [ADR-0032](0032-lobby-chat-guest-profile.md) | 大廳聊天訪客職業/等級顯示複用既有 quick-guest Redis profile 機制（`SendLobbyChatMessage` 接上 `EnsureQuickGuestProfile`／cookie，渲染短路條件改為職業與等級皆完整才顯示），推翻方案 B（獨立訪客機制）與方案 C（前端直傳無 session）。編號跳過 0031——該號已被另一支並行分支（公會設定入口）搶先使用，避免撞號 | Accepted | 2026-07-11 | backend / notify / party；frontend / chat / party |
