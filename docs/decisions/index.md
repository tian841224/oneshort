...
| ADR-0018 | 桌面版建立隊伍改為導覽式 wizard：`useWizardFieldErrors`/`CreatePartyWizardNav` 共用件、`.os-create-party-layout` 三欄變兩欄。本文原本獨立記錄於本分支，因與同源分支 `docs/guest-party-followup-bugfixes` 針對同一支 `feature/create-party-desktop-wizard` 前端分支各自撰寫的 ADR-0018 撞號，兩者描述同一決策，已將本文獨有細節（`CreatePartyFieldName`/`CreatePartyGuestFieldName` 泛型參數、`isMobile` 從 `scrollToField` 移除、560px 表單寬度理由、訪客已有身分時桌面自動跳頁邏輯）合併進對方分支的權威版本，本檔案已移除。權威版本見 `docs/guest-party-followup-bugfixes` 分支的 `docs/decisions/0018-create-party-desktop-wizard.md`。 | Superseded by `docs/guest-party-followup-bugfixes` 分支的 ADR-0018 | 2026-07-09 | frontend-rwd / party |
