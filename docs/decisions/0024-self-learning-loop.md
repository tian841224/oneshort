# ADR-0024: 自我學習迴圈的架構、全域／專案分層與記憶儲存位置
...
> **編號變更說明**：本文原本在 `docs/self-learning-loop` 分支上獨立編號為 ADR-0012，合併回 develop 時發現與已合併的 [ADR-0012](0012-guide-widget-write-serialization.md)（guide-widget 寫入序列化，主題完全不同）撞號，依 `docs/decisions/index.md` 命名規則改編號為 ADR-0024（依 develop 當下最新流水號遞增），檔案同步從 `0012-self-learning-loop.md` 重新命名為 `0024-self-learning-loop.md`。內容本身未變更。
